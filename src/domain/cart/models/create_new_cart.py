...
async def create_open_order(order_collection, user_id):
    opened_order = await get_opened_order_by_user_id(order_collection, user_id)
    if opened_order is None:
        ...
        opened_order = await create_order(order_collection, order)
    return opened_order
...
async def get_opened_order_by_user_id(order_collection, user_id):
    try:
        order = await order_collection.find_one({'user_id': user_id, 'opened': True})
        if order:
            return order
    except Exception as e:
        print(f'get_order.error: {e}')

async def validate_order_by_email_user(order_collection, email):
	try:
		order_cursor =  order_collection.aggregate([
			{
...
async def insert_product_in_order(order_collection, products: dict, opened_order):
    ...
    order = await order_collection.update_one(filter, update_product)
  
    return order


async def closed_order(order_collection, opened):
    filter = {
        opened: True
    }
    updated_record = {
    ...
    order = await order_collection.update_one(filter, updated_record)
    return order
